        ...
        {mainTab === 'history' ? (
          ...
          <div>
            {loading && processedOrders.length === 0 ? (
              ...
              /* KDS Grid View Mode - Industry Standard Uniform Height Grid */
                ...
                <AnimatePresence>
                  {processedOrders.map((order) => {
                    ...
                    const statusAccentClass = 
                      order.status === ORDER_STATUS.PREPARING ? 'border-t-4 border-t-orange-500' :
                      order.status === ORDER_STATUS.READY ? 'border-t-4 border-t-emerald-500' :
                      order.status === ORDER_STATUS.SERVED ? 'border-t-4 border-t-purple-600' :
                      order.status === ORDER_STATUS.BILL_REQUESTED ? 'border-t-4 border-t-amber-500' :
                      order.status === ORDER_STATUS.FINISHED ? 'border-t-4 border-t-slate-300 opacity-80' :
                      order.status === ORDER_STATUS.CANCELLED ? 'border-t-4 border-t-rose-500 opacity-75' :
                    ...
                    return (
                      ...
                      >
                        <Card className={`h-full overflow-hidden border border-slate-200/90 rounded-2xl shadow-xs hover:shadow-md transition-all flex flex-col justify-between bg-white ${statusAccentClass}`}>
                          ...
                          <div>
                            <div className="p-3 px-3.5 border-b border-slate-100 flex items-start justify-between gap-2 bg-slate-50/50">
                              ...
                              <div className="flex flex-col items-end gap-1 shrink-0">
                                ...
                                <div className={`flex items-center gap-1 text-[9.5px] font-bold px-2 py-0.5 rounded-full ${
                            ...
                            <div className="p-3 px-3.5 bg-white">
                              ...
                              <div className="space-y-1.5 min-h-[75px] max-h-[140px] overflow-y-auto pr-0.5">
                                {items.length === 0 ? (
