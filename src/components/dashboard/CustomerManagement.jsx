...
const exportPDF = async (list, stats = {}, restaurantName = "Tiger Bistro") => {
  const {
    ...
    reportTitle: "Customer Intelligence Report",
    reportSubtitle:
      "Guest Lifetime Value, retention dynamics, and behavioral loyalty audit",
    tagline: "SERVORA INTELLIGENCE SUITE  •  GUEST LIFETIME VALUE AUDIT",
    restaurantName,
    badgeText: "VERIFIED CRM",
    timeRangeLabel: "LIFETIME CRM",
    primaryAccent: [13, 148, 136], // Teal-600
    secondaryAccent: [245, 158, 11], // Amber-500
    watermarkSubtext: "CUSTOMER INTELLIGENCE • VERIFIED CRM",
  });

  // ── 3. Section Title 1: CRM Metrics ──
  const sec1Y = 51;
  ...
  doc.text("EXECUTIVE GUEST LIFETIME & RETENTION METRICS", 19, sec1Y + 4);
  ...
  const kpis = [
    {
      pill: "TOTAL PROFILES",
      val: `${stats.total || list.length || 0} Guests`,
      sub: `${stats.newThisMonth || 0} New This Month`,
      subColor: [5, 150, 105],
      accent: [13, 148, 136], // Teal
    },
    {
      pill: "LIFETIME REVENUE",
      val: fmtPdfCurrency(stats.totalSpent || 0),
      sub: `Avg ${fmtPdfCurrency(stats.avgSpent || 0)} / guest`,
      subColor: [5, 150, 105],
      accent: [245, 158, 11], // Amber
    },
    {
      pill: "RETENTION RATE",
      val: `${stats.retentionRate || 0}%`,
      sub: `${stats.repeatCustomers || 0} Repeat Diners`,
      subColor: [37, 99, 235],
      accent: [99, 102, 241], // Indigo
    },
    {
      pill: "CHURN AT RISK",
      val: `${stats.atRiskCount || 0} Guests`,
      sub: "Action Required",
      subColor: [225, 29, 72],
  ...
  // ── 5. Section Title 2: Customer Ledger ──
  const sec2Y = 87;
  ...
  doc.setFontSize(9);
  doc.setFont("helvetica", "bold");
  doc.text("VERIFIED GUEST DIRECTORY & AUDIT LEDGER", 19, sec2Y + 4);

  const countBadge = `${list.length} Records Verified`;
  doc.setFontSize(7);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100, 116, 139);
  doc.text(countBadge, pageWidth - 14, sec2Y + 4, { align: "right" });

  // ── 6. High-Impact Table via autoTable ──
  const tableHeaders = [
    [
      "#",
      "Guest Identity",
  ...
  const tableRows = list.map((c, idx) => [
    String(idx + 1).padStart(2, "0"),
    c.name || "Anonymous Guest",
    c.phone ? `+91 ${c.phone}` : "Not Provided",
    c.tag || "Regular",
    String(c.visits || 1),
    fmtPdfCurrency(c.totalSpent || 0),
    `${loyaltyScore(c)}/100`,
    c.health || "Active",
  ]);
  ...
  autoTable(doc, {
    ...
    headStyles: {
      fillColor: [11, 15, 25], // Midnight Slate Header
      ...
      <CardContent className="p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          {badge && (
            <Badge
              variant="outline"
              className={cn("text-[10px] font-bold", textCls)}
            >
              {badge}
            </Badge>
          )}
...
const CustomerProfileDialog = ({ customer, children }) => {
  ...
  const metrics = [
    ...
    { label: "Total Visits", value: customer.visits, icon: Repeat2 },
    {
      ...
      value:
        customer.visits > 0
          ? fmtCurrency(customer.totalSpent / customer.visits)
          : "₹0",
      icon: ShoppingBag,
    },
    {
      label: "Days Since Visit",
      value: daysSince(customer.lastVisit),
      icon: Calendar,
    },
  ];
  return (
      ...
      <DialogContent className="max-w-2xl bg-white p-0 overflow-hidden border-0 shadow-2xl rounded-2xl">
        ...
        <div className="bg-linear-to-br from-slate-800 to-slate-900 p-6 text-white relative overflow-hidden">
          ...
          <div className="relative flex items-center gap-5">
            <div
              className={cn(
                "w-16 h-16 rounded-2xl bg-linear-to-br flex items-center justify-center text-2xl font-black text-white shadow-xl",
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                ...
                <Badge
                  variant="outline"
                  className={cn(
                    "border-white/20 text-white/80 text-xs",
              ...
              <div className="flex flex-wrap gap-2">
                {customer.email && (
                  <a
                    ...
                    className="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-xl border border-gray-100 text-sm font-medium text-gray-700 hover:bg-teal-50 hover:text-teal-700 transition-colors"
                  >
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-sm font-black text-gray-900">
                          {fmtCurrency(order.total || order.revenue)}
                        </p>
                        <Badge
                          variant="outline"
                          className="text-[10px] h-4 mt-0.5 border-teal-100 text-teal-600"
                        >
                          Dine-In
                        </Badge>
                      </div>
...
const CustomerManagement = ({
  ...
  const restaurantName =
    profile?.name || profile?.business_name || "Tiger Bistro";
  ...
  const { customers, chartData, stats } = useMemo(() => {
    ...
    const buildFromDB = rawCustomers
      .map((c) => {
    ...
    history.forEach((order) => {
      ...
      if (new Date(dateRaw) > new Date(map[name].lastVisit))
        map[name].lastVisit = dateRaw;
    });
    const fallbackList = Object.values(map)
      .map((c) => {
        let tag = "New";
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      last7Days.push({
        ...
        count: dailySignups[k] || 0,
      });
    }
    const retentionRate =
      finalList.length > 0
        ? (finalList.filter((c) => c.visits > 1).length / finalList.length) *
          100
        : 0;
    const totalRevenue = finalList.reduce((s, c) => s + (c.totalSpent || 0), 0);
    const totalVisits = Math.max(
      finalList.reduce((s, c) => s + c.visits, 0),
      1,
    );
  ...
  const filteredCustomers = useMemo(() => {
    let list = customers.filter((c) => {
      ...
      const matchTier =
        ...
        c.tag === tierFilter ||
        (tierFilter === "At Risk" && c.health === "At Risk");
  ...
  const tierRevenueData = useMemo(() => {
    ...
    customers.forEach((c) => {
      map[c.tag] = (map[c.tag] || 0) + c.totalSpent;
    });
  ...
  const atRiskCustomers = useMemo(
    () => customers.filter((c) => c.health === "At Risk").slice(0, 4),
    [customers],
  );
    ...
    <div className="hidden lg:block sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-200/50 px-8 py-5 mb-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex -space-x-2">
            {customers.slice(0, 4).map((c, i) => (
              <div
                key={i}
                className={cn(
                  "w-8 h-8 rounded-full border-2 border-white bg-linear-to-br flex items-center justify-center text-[10px] font-bold text-white shadow-sm",
                  avatarColor(c.name),
                )}
              >
                {c.name.charAt(0)}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="rounded-xl border-gray-200 h-10 font-semibold shadow-none"
              >
            ...
            <DropdownMenuContent align="end" className="w-40 rounded-xl">
              ...
              <DropdownMenuItem
                onClick={() => exportPDF(customers, stats, restaurantName)}
                className="font-medium cursor-pointer text-teal-600 focus:text-teal-700"
              >
                Export as PDF
              </DropdownMenuItem>
            </DropdownMenuContent>
          ...
          <Button
            size="sm"
            className="rounded-xl bg-teal-600 hover:bg-teal-700 text-white h-10 font-semibold shadow-none"
            onClick={() => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <StatCard
          ...
          value={fmtCurrency(stats.avgOrderVal)}
          color="violet"
          badge="AOV"
        />
        <StatCard
        ...
        <Card className="lg:col-span-2 border-0 shadow-sm ring-1 ring-gray-100 rounded-2xl overflow-hidden">
          <CardHeader className="border-b border-gray-50 p-5">
            ...
            <CardDescription className="text-xs">
              New customers over the last 7 days
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="p-4 flex flex-col items-center">
            ...
            <div className="w-full grid grid-cols-2 gap-2 mt-2">
              {[
                ...
                { label: "Regular", color: "bg-blue-500" },
                { label: "New", color: "bg-teal-500" },
                  ...
                  return (
                    <CustomerProfileDialog key={c.id} customer={c}>
                      <div className="flex items-center gap-4 p-4 hover:bg-gray-50/70 cursor-pointer transition-colors group">
                        ...
                        <div
                          className={cn(
                            "w-10 h-10 rounded-xl bg-linear-to-br flex items-center justify-center text-sm font-black text-white shadow-sm",
                            avatarColor(c.name),
                          )}
                        >
                          {c.name.charAt(0)}
          ...
          <CardContent className="p-4 space-y-3">
            {atRiskCustomers.length === 0 ? (
              ...
              atRiskCustomers.map((c, i) => (
                ...
                >
                  <div
                    className={cn(
                      "w-9 h-9 rounded-xl bg-linear-to-br flex items-center justify-center text-sm font-black text-white shadow-sm shrink-0",
                      avatarColor(c.name),
                    )}
                  >
                    {c.name.charAt(0)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-red-500 font-semibold">
                      {daysSince(c.lastVisit)}d since last visit
                    </p>
                  </div>
                  <Badge
                    variant="outline"
                    className="text-[10px] border-red-200 text-red-600 shrink-0"
                  >
                    Risk
                  </Badge>
                </div>
            ...
            {atRiskCustomers.length > 0 && (
              <Button
                size="sm"
                variant="outline"
                className="w-full rounded-xl border-red-100 text-red-600 hover:bg-red-50 text-xs font-semibold"
              >
      ...
      {tierRevenueData.length > 0 && (
        <Card className="border-0 shadow-sm ring-1 ring-gray-100 rounded-2xl overflow-hidden">
          <CardHeader className="border-b border-gray-50 p-5">
            ...
            <CardDescription className="text-xs">
              Lifetime revenue breakdown across loyalty tiers
            </CardDescription>
          </CardHeader>
                  ...
                  <Bar dataKey="value" radius={[6, 6, 0, 0]}>
                    {tierRevenueData.map((_, i) => (
                      <Cell
                        key={i}
                        fill={
                          ["#f59e0b", "#3b82f6", "#0d9488", "#ef4444"][i % 4]
                        }
                      />
                    ))}
                  </Bar>
    ...
    const segs = [
      {
        key: "VIP",
        label: "VIP Champions",
        desc: "High spend & frequent visitors",
        icon: Crown,
        color: "amber",
        count: stats.vip,
        revenue: customers
      ...
      },
      {
        ...
        desc: "Multiple repeat visits",
        icon: Repeat2,
        color: "blue",
        count: stats.regular,
        revenue: customers
          .filter((c) => c.tag === "Regular")
          .reduce((s, c) => s + c.totalSpent, 0),
      },
      {
        key: "New",
        label: "New Customers",
        desc: "First-time or single visit",
        icon: UserPlus,
        color: "teal",
        count: stats.new,
        revenue: customers
      ...
      },
      {
        key: "At Risk",
        label: "At-Risk Customers",
        desc: "Haven't visited in 45+ days",
        icon: AlertTriangle,
        color: "red",
        count: stats.atRisk,
        revenue: customers
          .filter((c) => c.health === "At Risk")
          .reduce((s, c) => s + c.totalSpent, 0),
      },
    ];
    const CM = {
      ...
      teal: {
        card: "border-teal-100 bg-teal-50/30",
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {segs.map((seg) => {
            ...
            const segC = customers.filter((cu) =>
              seg.key === "At Risk"
                ? cu.health === "At Risk"
                : cu.tag === seg.key,
            );
                ...
                <CardContent className="p-5 space-y-4">
                  ...
                  <div>
                    <div className="flex justify-between text-[10px] text-gray-400 font-semibold mb-1">
                      <span>Share of customer base</span>
                      <span>{pct}%</span>
                    </div>
                    ...
                    <div className="flex -space-x-2">
                      {segC.slice(0, 5).map((cu, i) => (
                        <div
                          key={i}
                          className={cn(
                            "w-7 h-7 rounded-full border-2 border-white bg-linear-to-br flex items-center justify-center text-[9px] font-black text-white",
                            avatarColor(cu.name),
                          )}
                        >
                          {cu.name.charAt(0)}
        ...
        <Card className="border-0 shadow-sm ring-1 ring-gray-100 rounded-2xl overflow-hidden">
          <CardHeader className="border-b border-gray-50 p-5">
            ...
            <CardDescription className="text-xs">
              Average lifetime value per customer by tier
            </CardDescription>
          </CardHeader>
          <CardContent className="p-5 space-y-4">
            {segs.map((seg) => {
              ...
              const maxLTV = Math.max(
                ...segs.map((s) => (s.count > 0 ? s.revenue / s.count : 0)),
                1,
              );
  ...
  const renderInsights = () => {
    ...
    const healthyPct =
      stats.total > 0
        ? Math.round(((stats.total - stats.atRisk) / stats.total) * 100)
        : 0;
    const vipRevenuePct =
      stats.total > 0
        ? Math.round(
    ...
    const radarData = [
      {
        subject: "Retention",
        A: parseFloat(String(stats.retention || "0")),
        fullMark: 100,
      },
      {
        subject: "VIP %",
        A: stats.total > 0 ? Math.round((stats.vip / stats.total) * 100) : 0,
        fullMark: 100,
      },
      { subject: "Health", A: healthyPct, fullMark: 100 },
      {
        subject: "Engagement",
        A: Math.min(parseFloat(String(avgVisitFreq)) * 10, 100),
        fullMark: 100,
      },
      {
        subject: "LTV Index",
        A: Math.min(Math.round(stats.avgOrderVal / 1000), 100),
        fullMark: 100,
      },
    ];
    return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            {[
              {
                icon: UserCheck,
                title: "Loyalty Program Health",
                desc: `${stats.vip} VIP members driving ${vipRevenuePct}% of revenue`,
              ...
              },
              {
                ...
                desc: `${stats.retention}% of customers have repeat visits — above the 60% industry average`,
                accent: "border-teal-100 bg-teal-50/30",
                iconCls: "bg-teal-100 text-teal-600",
              },
              {
                icon: AlertTriangle,
                title: "Churn Risk Alert",
                desc: `${churnRisk} customer${churnRisk !== 1 ? "s" : ""} at risk. Re-engage within 7 days to recover revenue.`,
              ...
              },
              {
                ...
                desc: `Converting ${stats.new} new customers to regulars could increase LTV by an estimated 3–5×`,
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="rounded-xl border-gray-200 shadow-none font-semibold"
                >
                  <Download className="w-4 h-4 mr-2" />
                  Export
                </Button>
              ...
              <DropdownMenuContent align="end" className="w-40 rounded-xl">
                ...
                <DropdownMenuItem
                  onClick={() =>
                    exportPDF(filteredCustomers, stats, restaurantName)
                  }
                  className="font-medium cursor-pointer text-teal-600 focus:text-teal-700"
                >
                  Export as PDF
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-gray-50 bg-gray-50/50">
                <TableHead className="font-bold text-gray-500 text-xs">
                  Customer
                </TableHead>
                <TableHead className="font-bold text-gray-500 text-xs">
                  Tier
                ...
                <TableHead
                  className="font-bold text-gray-500 text-xs cursor-pointer hover:text-teal-600"
                ...
                <TableHead
                  className="font-bold text-gray-500 text-xs cursor-pointer hover:text-teal-600 text-right"
              ...
              {filteredCustomers.length === 0 ? (
                ...
                filteredCustomers.map((customer, idx) => {
                  ...
                  return (
                    <TableRow
                      key={idx}
                      className="group hover:bg-gray-50/70 transition-colors border-gray-50"
                    >
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <div
                            className={cn(
                              "w-9 h-9 rounded-xl bg-linear-to-br flex items-center justify-center text-sm font-black text-white shadow-sm",
                              avatarColor(customer.name),
                            )}
                          >
                            {customer.name.charAt(0)}
                          ...
                          <div>
                            ...
                            <div className="text-[11px] text-gray-400 truncate max-w-40">
                              {customer.email || customer.phone || "—"}
                            </div>
                          </div>
                        </div>
                      ...
                      <TableCell>
                        ...
                        <div className="text-[10px] text-gray-400">
                          {daysSince(customer.lastVisit)}d ago
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <CustomerProfileDialog customer={customer}>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="w-8 h-8 opacity-0 group-hover:opacity-100 text-gray-400 hover:text-teal-600 hover:bg-teal-50 transition-all"
                          >
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 w-full pb-32 lg:pb-12 relative overflow-hidden">
      {!isPremium && (
        <PremiumLock navigate={navigate} setActiveItem={setActiveItem} />
      )}
      <div
        className={cn(
          "transition-all duration-700",
          !isPremium &&
            "blur-xl grayscale-[0.5] opacity-50 pointer-events-none scale-[0.98]",
        )}
      >
        {renderHeader()}
          ...
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
              <TabsList className="bg-white border p-1 h-11 rounded-xl shadow-sm border-gray-100 w-full sm:w-auto overflow-x-auto shrink-0">
                <TabsTrigger
                  value="overview"
                  className="rounded-lg px-4 data-[state=active]:bg-teal-600 data-[state=active]:text-white shadow-none transition-all text-xs font-semibold gap-1.5"
                >
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                  ...
                  <Select value={tierFilter} onValueChange={setTierFilter}>
                    ...
                    <SelectContent className="bg-white border border-gray-100 shadow-2xl rounded-2xl p-1">
                      ...
                      <SelectItem
                        value="VIP"
                        className="rounded-xl font-bold py-2.5 text-sm text-amber-600"
                      >
                        VIP Members
                      </SelectItem>
                      <SelectItem
                        value="Regular"
                        className="rounded-xl font-bold py-2.5 text-sm text-blue-600"
                      >
                        Regulars
                      </SelectItem>
                      <SelectItem
                        value="New"
                        className="rounded-xl font-bold py-2.5 text-sm text-teal-600"
                      >
                        New Signups
                      </SelectItem>
                      <SelectItem
                        value="At Risk"
                        className="rounded-xl font-bold py-2.5 text-sm text-red-600"
                      >
                        At Risk
                      </SelectItem>
                    </SelectContent>
            ...
            <TabsContent
              value="overview"
              className="shadow-none border-0 focus-visible:outline-none"
            >
              {renderOverview()}
            </TabsContent>
